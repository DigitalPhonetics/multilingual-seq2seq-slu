...
use_lid=false              # Whether to use language id as the inputs (Need utt2lid in data directory).
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if "${use_lid}"; then
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/utt2lid,lids,text_int "
            _opts+="--valid_data_path_and_name_and_type ${_asr_valid_dir}/utt2lid,lids,text_int "
        fi
